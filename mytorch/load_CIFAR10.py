...
def read_data():
    ...
    train_labels = np.zeros((batch_size * 5, ), dtype=np.int32)
    ...
    for i in range(5):
        ...
        train_labels[np.arange(i * batch_size, (i + 1) * batch_size)] = dict[b'labels']
    filename = 'test_batch'
    filename = os.path.join(CIFAR_DIR, filename)
    dict = unpickle(filename)
    test_data = np.asarray(dict[b'data'])
    test_labels = np.zeros((batch_size, ), dtype=np.int32)
    test_labels[np.arange(batch_size)] = dict[b'labels']
    return train_data.reshape((-1, 3, 32, 32)), train_labels,\
           test_data.reshape((-1, 3, 32, 32)), test_labels
